...
- `assembly-orientation.svg` — the figure to have open while soldering. Both
  sides of the board with every designator and every value, and callouts for
  the five parts that can go in backwards. Generated by
  `tools/render-assembly.py`; the geometry comes from the board and the
  polarity from the **netlist** — the LED cathode is whichever pad is on SGND,
...
- `solder-check.md` — the other half of the same job, after the iron rather
  ...
  each with the copper-to-copper gap. Generated by `tools/render-solder-check.py`
  from the netlist and the **filled** copper, so the pour is in it — which is
  the point, since for most pads the nearest copper is ground at the zone
  clearance and not the next pad along. A table of forty clearances kept by
  hand would be wrong after one re-layout with no way to tell.

  Two traps it was written around, both found while writing it: a circular pad
  is not a square one, and treating it as such understates every gap by the
  corner (0.33 mm on a 1.6 mm pad, which is the whole margin); and
  `PCB_VIA::GetWidth()` wants a layer argument in KiCad 10 and **asserts**
  without one, which on a headless machine is a modal dialog and looks exactly
  like an infinite loop. Take the diameter off the bounding box instead
